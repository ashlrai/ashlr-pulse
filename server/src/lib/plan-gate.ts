...
import type { OrgRow } from "./org-db";
...
export function limitsFor(org: Pick<OrgRow, "plan" | "subscription_status">): PlanLimits {
  // Past-due, canceled, or incomplete subscriptions revert to free
  // limits so we don't ship features for nonpayment. Trialing users
  // get full Pro limits — that's the point of the trial.
...
export function requirePlan(
  org: Pick<OrgRow, "plan" | "subscription_status">,
  minimum: "pro" | "team",
): void {
  const tier = org.plan;
