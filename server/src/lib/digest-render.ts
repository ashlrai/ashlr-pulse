...
export interface DigestRenderOptions {
  ...
  /** AI-generated standup. Renders as Yesterday/Today/Blocked block when present. */
  standup?: { yesterday: string; today: string; blocked: string } | null;
  /** Monday-only weekly recap paragraph. */
  weeklyRecap?: { body: string } | null;
}
...
function renderText(d: DigestPayload, opts: DigestRenderOptions): string {
  ...
  lines.push("");
  if (opts.weeklyRecap) {
    lines.push("LAST WEEK · RECAP");
    lines.push(opts.weeklyRecap.body);
    lines.push("");
  }
  if (opts.briefing) {
  ...
  if (opts.standup) {
    lines.push("STANDUP");
    lines.push(`  yesterday: ${opts.standup.yesterday}`);
    lines.push(`  today:     ${opts.standup.today}`);
    if (opts.standup.blocked) lines.push(`  blocked:   ${opts.standup.blocked}`);
    lines.push("");
  }
  ...
  if (d.peers.length) {
    ...
    for (const p of d.peers) {
      ...
      lines.push(`  ${peerSummaryLine(p)}`);
      if (p.byProject.length) {
        lines.push("  projects:");
        for (const proj of p.byProject.slice(0, 5)) {
          lines.push(`    ${pad(proj.project_name, 24)} ${pad(`${proj.repos.length} repos`, 10)} ${pad(fmtTokens(proj.tokens), 8)} ${p.showCost ? fmtUsd(proj.cents) : ""}`);
        }
      }
...
function renderHtml(d: DigestPayload, opts: DigestRenderOptions): string {
  const styles = `
    ...
    .peer { margin-top: 8px; }
    .peer-email { color: ${COLORS.magenta}; font-size: 11px; margin-bottom: 4px; }
    .peer-summary { color: ${COLORS.dim}; font-size: 11px; margin-bottom: 10px; font-variant-numeric: tabular-nums; }
  ...
  const standup = opts.standup
    ? `<div class="standup">
        <div class="standup-label">standup</div>
        <div class="standup-row"><span class="standup-key">yesterday</span><span>${esc(opts.standup.yesterday)}</span></div>
        <div class="standup-row"><span class="standup-key">today</span><span>${esc(opts.standup.today)}</span></div>
        ${opts.standup.blocked
          ? `<div class="standup-row"><span class="standup-key" style="color:${COLORS.amber}">blocked</span><span>${esc(opts.standup.blocked)}</span></div>`
          : ""}
      </div>`
    : "";

  const recap = opts.weeklyRecap
    ? `<div class="recap">
        <span class="recap-label">last week · recap</span>
        ${esc(opts.weeklyRecap.body)}
      </div>`
    : "";
...
function renderPeersBlockHtml(peers: DigestPeer[]): string {
  const visible = peers.filter(
    (p) => p.bySource.length || (p.byRepo ?? []).length || p.totals.events > 0,
  );
  if (!visible.length) return "";

  const blocks = visible.map((p) => {
    const summary = `<div class="peer-summary">${esc(peerSummaryLine(p))}</div>`;
    const projectTbl = p.byProject.length
      ? tableByProject(p.byProject.slice(0, 5))
      : "";
...
function peerSummaryLine(p: DigestPeer): string {
  const parts: string[] = [];
  parts.push(`${p.totals.events} event${p.totals.events === 1 ? "" : "s"}`);
  if (p.showCost && p.totals.tokens > 0) parts.push(`${fmtTokens(p.totals.tokens)} tok`);
  if (p.showCost && p.totals.cents != null && p.totals.cents > 0) parts.push(fmtUsd(p.totals.cents));
  if (p.totals.repos != null) parts.push(`${p.totals.repos} repo${p.totals.repos === 1 ? "" : "s"}`);
  if (p.totals.tools != null) parts.push(`${p.totals.tools} tool${p.totals.tools === 1 ? "" : "s"}`);
  return parts.join(" · ");
}
